...
for(let g of namespaces){
  ...
  let varName = g.namespace.length === 0 ? '$' : g.namespace.join('$')
  let namespace = g.namespace.length === 0 ? `''` : `'${g.namespace.join('/')}'`
  exportInst += `export const ${varName} = {\n  namespace: ${namespace},\n`
  fs.writeSync(outputFd, exportInst, null, 'utf8')
  
  if(g.getters.length > 0){
    fs.writeSync(outputFd, '  getters: {\n', null, 'utf8')
   for(let getter of g.getters){
      let prop = `    ${getter[0]}: '${getter[1]}',\n`   
  ...
  if(g.actions.length > 0){
   fs.writeSync(outputFd, '  actions: {\n', null, 'utf8')
   for(let action of g.actions){
      let prop = `    ${action[0]}: '${action[1]}',\n`      
      fs.writeSync(outputFd, prop, null, 'utf8')
     }
     fs.writeSync(outputFd, '  },\n', null, 'utf8')
  }
  ...
  if(g.mutations.length > 0){
   fs.writeSync(outputFd, '  mutations: {\n', null, 'utf8')
   for(let mutation of g.mutations){
      let prop = `    ${mutation[0]}: '${mutation[1]}',\n`      
      fs.writeSync(outputFd, prop, null, 'utf8')
     }
     fs.writeSync(outputFd, '  },\n', null, 'utf8')
  }
